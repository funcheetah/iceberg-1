...
public class SparkValueReaders {
  ...
  /*
    In case of complex union, table schema (i.e. Iceberg schema) can possibly be pruned
    because of the column projection, while file schema (i.e. Avro schema) can not be pruned
    to make the data read from the file successfully. Therefore, table schema needs to be
    passed to ComplexUnionReader to help it only return the data of the projected type in the union.
  */
  static ValueReader<InternalRow> complexUnion(Schema schema, List<ValueReader<?>> readers, Type expected) {
